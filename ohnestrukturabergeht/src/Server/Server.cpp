...
std::string Server::handleGETRequest(const std::string& path) {
	...
	if (std::filesystem::is_directory(filePath) && !matchedRoute->directoryListing)
	{
		// 403 Forbidden
		try {
			std::string errorContent = readFile(_config.errorPages[403]);
			...
			// Fallback wenn die Error Page nicht geladen werden kann
			std::string defaultError = "<html><body><h1>403 Forbidden</h1></body></html>";
			return "HTTP/1.1 403 Forbidden\r\n"
				   "Content-Type: text/html\r\n"
				   "Content-Length: " + std::to_string(defaultError.length()) + "\r\n"
				   "\r\n" + defaultError;
    ...
    if (std::filesystem::is_directory(filePath)) {
    	if (matchedRoute->directoryListing) {
            ...
            return "HTTP/1.1 200 OK\r\n"
                   "Content-Type: text/html\r\n"
                   "Content-Length: " + std::to_string(listing.length()) + "\r\n"
                   "\r\n" + listing;
        }
    }
